...
| Flag | Label | Paste public? | Wiki edit? |
|------|-------|--------------|------------|
| 0 | 未实名 (Unverified) | No | No |
| 1 | 已实名 (Verified) | Yes (`flag >= 1`) | No |
| 2 | 老师 (Teacher) | Yes | No |
| 3 | 管理员 (Admin) | Yes | Yes (`flag === 3`) |
...
| `/oi33/wiki/create` | WikiEditHandler (GET/POST) | PRIV_USER_PROFILE (but body checks `realname_flag === 3`) |
| `/oi33/wiki/:id` | WikiShowHandler | public |
| `/oi33/wiki/:id/edit` | WikiEditHandler (GET/POST) | PRIV_USER_PROFILE (body checks `realname_flag === 3`) |
...
- Wiki editing requires **both** `PRIV_USER_PROFILE` (route-level) **and** `realname_flag === 3` (body-level check via `checkUserFlag` in `handler/utils.ts`).
