...
- Approval queue at `/oi33/requests` (admin only). Approve → `applyRequestPayload` applies the saved fields, sets `status=approved`. Reject → sets `status=rejected` + `rejectReason`.
